...
const latitude = document.querySelector('#latlong dd:first-of-type');
...
function success (position) {
  statusMsg.textContent = 'Geolocation successful!';
...
longitude.textContent = position.coords.longitude;
  ...
  let = `https://www.openstreetmap.org/#map=3/${position.coords.latitude}/${position.coords.longitude}`;
...
if(!navigator.geolocation) {
  ...
  statusMsg.textContent = 'Geolocation is not supported by your browser. Please try a different browser or device.';
} else {
  ...
  // STEP 3a: Use the getCurrentPosition() method, which passes the device position to a named callback function 
  // (if successful), or it calls an error function if it fails

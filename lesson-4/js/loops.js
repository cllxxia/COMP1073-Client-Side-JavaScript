...
btn.addEventListener('click', function() {
	...
	for (let i = 0; i < cities.length; i++) {
		...
		if (searchName.toLowerCase() === cities[i].toLowerCase()) {
			/* STEP 2c: If the city is found, output a message to the result paragraph and exit the loop */
			result.textContent = `${searchName} is in the list of cities.`;
			break; // Exit the loop
		} else {
			/* STEP 2d: If the city is not found, output a different message */
...
for (let i = 1; i <= num; i ++) {
	...
	if (Math.floor(sqRoot) !== sqRoot) {
		/* STEP 3b: If the square root of the number is not an integer, we don't want to output it to the paragraph,
		 so skip the rest of the instructions inside this loop and go back up to the FOR, using 'continue' */
